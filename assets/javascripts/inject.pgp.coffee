...
HELP="This message is PGP encrypted. Visit #{BASE_URL} to learn how to deal with it.\n\n"

CRYPTO_HEADER="-----BEGIN PGP MESSAGE-----" # "EnCt2"
CRYPTO_FOOTER="-----END PGP MESSAGE-----"   # "IwEmS"
...
class Popup
    ...
    constructor: ->
        request = createCORSRequest("POST", "http://localhost.:3000/encrypt")
        request.onload = ->
            alert request.responseText
        request.send("text=" + encodeURIComponent("Clean Text"))

        @cache = {}
        if @parse()
            if @encrypted
                @show "Decrypt PGP message","Decrypt", "Please start Encipher.It app and click 'Decrypt' button"
            else
                if @text
                    @show "Encrypt message with PGP","Encrypt", "Please start Encipher.It app and click 'Encrypt' button"
                else
